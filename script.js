...
const dershaneList = document.getElementById('dershaneler');
...
displayDershaneler(dershaneler);
...
filterBtn.addEventListener('click', () => {
    ...
    const hazirlikLGS = document.getElementById('hazirlikLGS').checked;
    const hazirlikYKS = document.getElementById('hazirlikYKS').checked;

    const hazirlikFiltre = [];
    if (hazirlikLGS) hazirlikFiltre.push("LGS");
    if (hazirlikYKS) hazirlikFiltre.push("YKS");

    const filtered = dershaneler.filter(dershane => {
        // Konum filtresi
        ...
        // Sınav Türü filtresi
        let hazirlikMatch = true;
        if (hazirlikFiltre.length > 0) {
            // Dershanenin hazırlık yaptığı sınavlardan en az birine uygun olup olmadığını kontrol et
            hazirlikMatch = hazirlikFiltre.some(h => dershane.hazirlik.includes(h));
        }

        return konumMatch && fiyatMatch && hazirlikMatch;
    });
